...
def setup_paths():
    ...
    return video_path_to_modify, output_video_path_after_modification, csv_path_for_data, log_path_from_logger


# Get paths and setup logging
_, _, _, log_path_for_config = setup_paths()  # needs here only for the logging.basicConfig
logging.basicConfig(level=logging.DEBUG, filename=log_path_for_config, filemode='w',
    ...
    while True:
        ...
        if ids is not None:
            ...
            for i, aruco_id in enumerate(ids):
                distance, yaw_angle, pitch_angle, roll_angle, \
                    translation_vector, rotation_vector = calculate_pose_and_distance(corners[i])
                ...
                csv_writer.writerow(
                    [frame_id, aruco_id[0],
                     ...
                     f"[{distance}, {yaw_angle}, {pitch_angle}]"])
                logging.info(
                    f"Frame {frame_id}: Aruco ID {aruco_id[0]} detected with distance {distance},"
                    f" yaw angle {yaw_angle}, pitch angle {pitch_angle}, roll angle {roll_angle}.")
...
def show_output_video(output_video_path_result):
    ...
    Args:
        output_video_path_result (str): Path to the output video file.
    """
    ...
    while True:
        ...
        if key == ord('q') or key == ord('e') or key == 'q' or key == 'e':  # Press 'q' or 'e'
            # to quit the video display early
            break
        elif key == ord('p') or key == ord(' ') or key == ' ' or key == 'p':  # Press 'p' or 'space'
            # to pause/resume the video display
            paused = not paused
        elif key == ord('d') or key == ord('D') or key == 'd' or key == 'D':  # Press 'd'
            # to step forward one frame when paused
            paused = True
            ret, frame = cap.read()
            if not ret:
                break
